...
export interface LinkIssuesInput {
  type: string;
  // Fiche BLOQUEUSE (pour "Blocks", verbe « blocks »).
  blockerKey: string;
  // Fiche BLOQUÉE (pour "Blocks", « is blocked by »).
  blockedKey: string;
}

// Crée un lien entre deux fiches via POST /issueLink.
//
// ATTENTION, mapping contre-intuitif vérifié en réel contre l'UI Jira Cloud :
// pour le type "Blocks", c'est l'`inwardIssue` qui EST le bloqueur (rendu
// « blocks ») et l'`outwardIssue` qui est le bloqué (rendu « is blocked by »).
// On envoie donc le bloqueur en inwardIssue et le bloqué en outwardIssue.
export async function linkIssues(
  ...
  await request(opts, "POST", "/issueLink", {
    body: {
      ...
      inwardIssue: { key: input.blockerKey },
      outwardIssue: { key: input.blockedKey },
    },
    parseJson: false,
  });
}
...
// Enfants d'une fiche (epic → stories, story → sous-tâches…) via JQL
// `parent = KEY`. Endpoint de recherche moderne de Jira Cloud (`/search/jql`).
export async function getChildIssues(
  opts: JiraClientOptions,
  key: string,
): Promise<ChildIssue[]> {
  const json = await request(opts, "POST", "/search/jql", {
    body: {
  ...
  return SearchResponseSchema.parse(json).issues;
}
