...
// Extrait la clé de projet d'une clé d'issue : "ABC-1812" -> "ABC".
...
function parseBlockSpec(
  spec: string,
  editedKey: string,
): { blockerKey: string; blockedKey: string } {
  const s = spec.trim();
  const invalid = () =>
    new Error(
  ...
  if (s.startsWith(">")) {
    ...
    return { blockerKey: editedKey, blockedKey: other };
  }
  if (s.endsWith(">")) {
    // "AUTRE>" : AUTRE bloque la fiche éditée.
    const other = s.slice(0, -1).trim();
    if (!other) throw invalid();
    return { blockerKey: other, blockedKey: editedKey };
  }
  throw invalid();
}
...
export function buildCli(): Command {
  ...
  program
    ...
    .option("--epic <KEY>", "Rattacher à une epic (clé de l'epic, ex: ABC-100)")
    .option(
      "--block <SPEC>",
    ...
    .action(async (opts) => {
      ...
      if (effectiveBlock) {
        const { blockerKey, blockedKey } = parseBlockSpec(
          effectiveBlock,
          created.key,
        );
        await linkIssues(jira, { type: "Blocks", blockerKey, blockedKey });
      }
  ...
  program
    .command("update")
    .argument("<issueKey>", "Clé de la fiche (ex: ABC-1234)")
  ...
  program
    .command("block")
    .argument("<issueKey>", "Clé de la fiche éditée (ex: ABC-1234)")
    .argument(
      "<spec>",
    ...
    .action(async (issueKey, spec) => {
      const config = loadConfig();
      const jira = jiraOptsFromConfig(config);
      const { blockerKey, blockedKey } = parseBlockSpec(spec, issueKey);
      await linkIssues(jira, { type: "Blocks", blockerKey, blockedKey });
      logger.success(`${blockerKey} bloque ${blockedKey}`);
    });
  ...
  program
    ...
    .action(async (issueKey, opts) => {
      ...
      // `children` = fiches dont `parent = issueKey` (enfants d'une epic, etc.).
      // Prop CALCULÉE côté CLI (JQL) : n'existe pas dans le modèle Jira et ne
      // peut donc pas être réécrite dans un ticket.
      const children = await getChildIssues(jira, issueKey);
          ...
          JSON.stringify(
            {
              ...
              // Prop calculée (JQL parent = key), non réinscriptible dans Jira.
              children: children.map((c) => ({
                key: c.key,
                summary: c.fields.summary ?? null,
                status: c.fields.status?.name ?? null,
                type: c.fields.issuetype?.name ?? null,
              })),
              links: links.map((l) =>
                l.outwardIssue
                  ? {
      ...
      if (children.length === 0) {
        ...
        for (const c of children) {
          const ct = c.fields.issuetype?.name ?? "";
          const cs = c.fields.status?.name ?? "?";
          console.log(
            `  • ${c.key} [${ct}] [${cs}] — ${c.fields.summary ?? ""}`,
          );
        }
      }
