...
// et liste les verbes métier + le fait qu'une simple clé de fiche (ABC-1234)
...
const SKILL_DESCRIPTION =
  ...
  "il fournit seulement une clé de fiche (ex. ABC-1234) sans nommer " +
...
const INVOCATION_NOTES = [
  ...
  "`get --json` produit une sortie machine " +
    ...
    "sprint, description, subtasks, children, links, comments, created, updated }` — " +
    "description et corps des commentaires convertis en texte brut (ADF → texte). " +
    "`children` liste les fiches dont `parent` = la fiche lue (stories d'une " +
    "epic, etc.) : propriété CALCULÉE par JQL, en LECTURE SEULE, absente du " +
    "modèle Jira et non réinscriptible dans un ticket.",
...
const USAGE_RULES = [
  ...
  "⚠️ DANGER `update` : la mise à jour ÉCRASE le titre et/ou la description " +
    "fournis (pas de fusion). Une description fournie REMPLACE INTÉGRALEMENT " +
    "l'ancienne — toute PIÈCE JOINTE INLINE (image/vidéo/média embarqué dans le " +
    "corps de la fiche via l'ADF) présente dans la description est alors " +
    "SUPPRIMÉE et la donnée est PERDUE DÉFINITIVEMENT (pas de restauration " +
    "possible). Avant tout `update` de description sur une fiche existante : " +
    "lire la fiche (`jira get <key>`), vérifier l'absence de média inline dans " +
    "la description, et si présent, préserver ces médias dans la nouvelle " +
    "description ou renoncer à écraser.",
...
const EXAMPLES: Record<string, string[]> = {
  get: ["jira get ABC-1234", "jira get ABC-1234 --json"],
  create: [
    ...
    'jira create -s "Titre" --block ">ABC-200"',
  ...
  epic: ["jira epic ABC-1234 ABC-100", "jira epic ABC-1234 none"],
  block: ['jira block ABC-1234 "ABC-100>"', 'jira block ABC-1234 ">ABC-200"'],
  comment: [
    'jira comment ABC-1234 "Reproduit en v2.3, priorité haute."',
    "jira comment ABC-1234 --file ./note.txt",
    "cat note.txt | jira comment ABC-1234 --file -",
  ],
  attach: ["jira attach ABC-1234 ./capture.png ./log.txt"],
  attachments: ["jira attachments ABC-1234"],
  download: [
    "jira download ABC-1234 capture.png",
    "jira download ABC-1234 10042",
    "jira download ABC-1234 all --out ./dl",
  ],
};
